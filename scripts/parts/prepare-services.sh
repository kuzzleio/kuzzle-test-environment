...
do
    echo -e "[$(date --rfc-3339 seconds)] - ${COLOR_YELLOW}Still trying to connect to elasticsearch at http://localhost:9200${COLOR_END}"
    sleep 1
done
# create a tmp index just to force the shards to init
curl -XPUT -s -o /dev/null "http://localhost:9200/%25___tmp"
echo -e "[$(date --rfc-3339 seconds)] - ${COLOR_BLUE}Elasticsearch is up. Waiting for shards to be active (can take a while)${COLOR_END}"
E=$(curl -s "http://localhost:9200/_cluster/health?wait_for_status=yellow&wait_for_active_shards=1&timeout=60s")
curl -XDELETE -s -o /dev/null "http://localhost:9200/%25___tmp"
